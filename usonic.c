...
// TIMER3 is Trigger PC5, echo PF4
#define TRIGGER2    (*((volatile uint32_t *)0x40006080))
#define TRIGGER2HIGH 0x20
#define ECHO2       (*((volatile uint32_t *)0x40025040))
#define ECHO2HIGH 0x10
...
//-------------------------Timer2-------------------------------
uint32_t Timer2First,Timer2Done,Timer2Pulse;
//-------------------------Timer3-------------------------------
...
void Timer0A_Handler(void){
  ...
  if(ECHO0 == ECHO0HIGH){ // first
    ...
    Timer0Done = 1;
		#if NUM_USONIC == 1
			OS_bSignal(&usonicDataReady);
		#elif NUM_USONIC == 2
			if(Timer1Done){
				OS_bSignal(&usonicDataReady);
			}
		#elif NUM_USONIC == 3
			if(Timer1Done && Timer2Done){
				OS_bSignal(&usonicDataReady);
			}
		#elif NUM_USONIC == 4
			if(Timer1Done && Timer2Done && Timer3Done){
				OS_bSignal(&usonicDataReady);
			}
		#endif
...
void Timer1A_Handler(void){
  ...
  if(ECHO1 == ECHO1HIGH){ // first
    ...
    Timer1Done = 1;
		...
		#elif NUM_USONIC == 3
			if(Timer0Done && Timer2Done){
				OS_bSignal(&usonicDataReady);
			}
		#elif NUM_USONIC == 4
			if(Timer0Done && Timer2Done && Timer3Done){
				OS_bSignal(&usonicDataReady);
			}
		#endif
  }
...
// the both edges of PF4 (T4CCP2).  The interrupt service routine
// acknowledges the interrupt records the time.
// PC5 GPIO output
// Input: none
// Output: none
void Timer2_Init(void){
  Timer2First = Timer2Done = Timer2Pulse = 0;
  ...
  GPIO_PORTC_DEN_R |= 0x20;        // enable PC5 as T3CCP0
	GPIO_PORTC_PCTL_R = (GPIO_PORTC_PCTL_R&0xFF0FFFFF);
  ...
  GPIO_PORTF_DEN_R |= 0x10;        // enable PF4 as T3CCP0
  GPIO_PORTF_PCTL_R = (GPIO_PORTF_PCTL_R&0xFFF0FFFF)+0x00070000;
  GPIO_PORTF_AMSEL_R &= ~0x10;     // disable analog functionality on PF4
  TIMER2_CTL_R &= ~TIMER_CTL_TAEN; // disable timer3A during setup
  TIMER2_CFG_R = TIMER_CFG_16_BIT; // configure for 16-bit timer mode
  TIMER2_TAMR_R = (TIMER_TAMR_TACMR|TIMER_TAMR_TAMR_CAP);   // 24-bit capture         
  TIMER2_CTL_R |= TIMER_CTL_TAEVENT_BOTH;// configure for both edges
  ...
  TIMER2_ICR_R = TIMER_ICR_CAECINT;// clear timer3A capture match flag
  TIMER2_CTL_R |= TIMER_CTL_TAEN;  // enable timer3A 16-b, +edge timing, interrupts
...
//------------Timer2_StartHCSR04------------
// start HCSR04 ultrasonic distance measurement
// 10us Pulse output on PC5 GPIO output
// Input: none
// Output: none
void Timer2_StartHCSR04(void){
  Timer2Done = 0;
  TRIGGER2 = TRIGGER2HIGH;
  DelayWait10us();
  DelayWait10us();
  TRIGGER2 = 0;
...
//------------Timer2_StartPing------------
// start Ping))) ultrasonic distance measurement
// 1) Make PF4 GPIO output
// 2) 5us Pulse output on PF4 GPIO output
// 3) Make PF4 input capture Timer input
// Input: none
// Output: none
void Timer2_StartPing(void){
  GPIO_PORTF_AFSEL_R &= ~0x10;     // disable alt funct on PF4
  GPIO_PORTF_PCTL_R = GPIO_PORTF_PCTL_R&0xFFF0FFFF;
  GPIO_PORTF_DIR_R |= 0x10;        // make F4 output
  Timer2Done = 0;
  ECHO2 = ECHO2HIGH;
  DelayWait5us();
  ECHO2 = 0;
  GPIO_PORTF_DIR_R &= ~0x10;       // make PF4 in
  GPIO_PORTF_AFSEL_R |= 0x10;      // enable alt funct on PF4
  GPIO_PORTF_PCTL_R = (GPIO_PORTF_PCTL_R&0xFFF0FFFFF)+0x00070000;
  TIMER2_ICR_R = TIMER_ICR_CAECINT;// clear timer2A capture match flag
}
//------------Timer2_Read------------
// read ultrasonic distance measurement
...
void Timer2A_Handler(void){
  TIMER2_ICR_R = TIMER_ICR_CAECINT;// acknowledge timer3A capture match
  if(ECHO2 == ECHO2HIGH){ // first
    Timer2First = TIMER2_TAR_R;
  }else{
    USONICValues[2] = (Timer2First - TIMER2_TAR_R)&0xFFFFFF;// 24 bits, 12.5ns resolution
    Timer2Done = 1;
		#if NUM_USONIC == 3
			if(Timer0Done && Timer1Done){
				OS_bSignal(&usonicDataReady);
			}
		#endif
		#if NUM_USONIC == 4
			if(Timer0Done && Timer1Done && Timer3Done){
				OS_bSignal(&usonicDataReady);
			}
		#endif
  }
...
void Timer3A_Handler(void){
  ...
  if(ECHO3 == ECHO3HIGH){ // first
    ...
    USONICValues[3] = (Timer3First - TIMER3_TAR_R)&0xFFFFFF;// 24 bits, 12.5ns resolution
    Timer3Done = 1;
		#if NUM_USONIC == 4
			if(Timer0Done && Timer1Done && Timer2Done){
				OS_bSignal(&usonicDataReady);
			}
		#endif
  }
